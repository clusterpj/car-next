...
const vehicleSchema = Yup.object().shape({
  ...
  modelName: Yup.string()
    ...
    .max(100, 'Model name must be at most 100 characters'), // Changed from 'model' to 'modelName'
  year: Yup.number()
  ...
  licensePlate: Yup.string()
    ...
    .matches(/^[A-Z0-9]{5,8}$/, 'Invalid license plate format'),
  vin: Yup.string()
  ...
  category: Yup.string()
    ...
    .oneOf(['economy', 'midsize', 'luxury', 'suv', 'van'], 'Invalid category'),
...
const validateVehicleData = async (
  ...
  try {
    const schemaKeys = Object.keys(vehicleSchema.fields) as Array<
      keyof typeof vehicleSchema.fields
    >
    const validKeys = Object.keys(data).filter(
      (key): key is keyof typeof vehicleSchema.fields =>
        schemaKeys.includes(key as any)
    )
...
async function handler(
  req: NextApiRequest,
  res: NextApiResponse<
    PaginatedVehicleResponse | IVehicle | ErrorResponse | SuccessResponse
  >
) {
  await dbConnect()
...
async function handleGet(
  ...
  if (modelName)
    query.modelName = new RegExp(sanitizeForRegex(modelName as string), 'i')
  ...
  const projection: Record<string, 1> = fields
    ? Object.fromEntries(
        (fields as string).split(',').map((field) => [field.trim(), 1])
      )
    : {}
...
async function handlePost(req: NextApiRequest): Promise<IVehicle> {
  ...
  const vehicleData: Partial<IVehicleProps> = {
    ...
    fuelType: sanitizedBody.fuelType as
      | 'gasoline'
      | 'diesel'
      | 'electric'
      | 'hybrid',
    transmission: sanitizedBody.transmission as 'automatic' | 'manual',
    category: sanitizedBody.category as
      | 'economy'
      | 'midsize'
      | 'luxury'
      | 'suv'
    ...
    features: Array.isArray(sanitizedBody.features)
      ? sanitizedBody.features.map(
          (feature) => sanitizeInput(feature) as string
        )
      : [],
    maintenanceHistory: Array.isArray(sanitizedBody.maintenanceHistory)
      ? sanitizedBody.maintenanceHistory.map((record) => {
          ...
          if (sanitizedDate instanceof Date) {
            ...
            typeof sanitizedDate === 'string' ||
            typeof sanitizedDate === 'number'
          ) {
  ...
  if (sanitizedBody.lastServiced) {
    ...
    if (typeof lastServiced === 'string' && !isNaN(Date.parse(lastServiced))) {
      ...
      vehicleData.nextServiceDue = calculateNextServiceDue(
        vehicleData.lastServiced
      )
    } else {
      throw new ApiError(400, 'Invalid lastServiced date')
    }
